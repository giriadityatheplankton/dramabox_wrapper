<script lang="ts">
    ...
    let activeEpisode = $state<any>(null);
...
<svelte:head>
    <title>{book.bookName} - Dwitabox</title>
</svelte:head>
...
<div class="hero">
    <div class="hero-bg" style="background-image: url({book.cover || book.coverWap})"></div>
    <div class="container hero-content">
        <div class="poster">
            <img src={book.cover || book.coverWap} alt={book.bookName} />
        </div>
        <div class="info">
            <h1>{book.bookName}</h1>
            <div class="meta">
        ...
        <div class="episodes-grid">
            {#each data.episodes as episode}
                <button 
                    ...
                    class="episode-card"
                    aria-label="Play Episode {episode.chapterName || episode.chapterIndex + 1}"
                >
                    <div class="episode-thumb">
                        <img src={episode.cover || book.cover || book.coverWap} alt="" />
                        <span class="play-icon">▶</span>
                    </div>
                    <div class="episode-info">
                        <h3>Episode {episode.chapterName || episode.chapterIndex + 1}</h3>
                    </div>
                </button>
            {/each}
        </div>
    ...
    <div class="modal-backdrop" onclick={closePlayer}>
        <div class="modal-content" onclick={(e) => e.stopPropagation()}>
            ...
            <div class="modal-info">
                <h3>Episode {activeEpisode.chapterName || activeEpisode.chapterIndex + 1}</h3>
            </div>
        </div>
    </div>
{/if}
